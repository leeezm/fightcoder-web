...
func (this ProblemUser) QueryByUserId(userId int64, size, start int) ([]*ProblemUser, error) {
	problemUser := make([]*ProblemUser, 0)
	err := OrmWeb.Where("user_id = ?", userId).Limit(size, start).Find(&problemUser)
	if err != nil {
		return nil, err
	}
	return problemUser, nil
...
func (this ProblemUser) Count(problemUser *ProblemUser) (int64, error) {
	sum, err := OrmWeb.Count(problemUser)
	if err != nil {
		return 0, err
	}
	return sum, nil
